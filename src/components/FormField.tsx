          ...
          <Select
            value={
              Array.isArray(field.value)
                ...
                : String(field.value ?? (initialValue ?? ""))
            }
            onValueChange={(val) => {
              field.onChange(val);
            }}
      ...
      case "file":
        return (
          <FilePond
            ...
            credits={false}
            acceptedFileTypes={accept ? accept.split(',').map(type => type.trim()) : ['image/*']}
            maxFiles={10}
            maxFileSize="10MB"
          />
        );
      ...
      case "checkbox-list": {
        ...
        return (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {options?.map((option) => (
              <label key={option.value} className="flex items-center gap-2 p-2 border border-gray-200 rounded-md cursor-pointer select-none">
                <input
                  type="checkbox"
